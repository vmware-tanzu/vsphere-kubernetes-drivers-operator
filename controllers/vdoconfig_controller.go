...
// +kubebuilder:rbac:groups="storage.k8s.io",resources=csinodes,verbs=create;get;list;watch;delete
// +kubebuilder:rbac:groups="storage.k8s.io",resources=csidrivers,verbs=create;update;patch;get;list;watch;delete;
...
// +kubebuilder:rbac:groups="apps",resources=daemonsets,verbs=get;list;create;update;patch;watch;delete;
	...
	if len(nodeName) > 0 && len(vdoConfig.Status.CPIStatus.NodeStatus) > 0 {
		if val, ok := vdoConfig.Status.CPIStatus.NodeStatus[nodeName]; ok {
			if val == vdov1alpha1.NodeStatusReady {
				vdoctx.Logger.Error(err, "Skipping Reconcile of VDOConfig resource %s generated by node events", "name", vdoConfig.Name)
...
func (r *VDOConfigReconciler) reconcileCSIDeploymentStatus(ctx vdocontext.VDOContext, clientset kubernetes.Interface) error {
	...
	err := r.fetchDaemonSetPodStatus(ctx, clientset, CSI_DAEMONSET_NAME, CsiNamespace, CSI_DAEMON_POD_KEY)
	if err != nil {
...
func (r *VDOConfigReconciler) CheckCompatAndRetrieveSpec(ctx vdocontext.VDOContext, req ctrl.Request, vdoConfig *vdov1alpha1.VDOConfig, matrixConfig string) error {
	...
	isCSINamespaceReq, err := r.compareVersions("2.3.0", r.CurrentCSIDeployedVersion, "100.0.0")
	if err != nil {
		return err
	}
	if isCSINamespaceReq {
		// create and update CSI Namespace
		CsiNamespace = "vmware-system-csi"
		err = r.createCSINamespace(ctx)
		if err != nil {
			ctx.Logger.Error(err, "Error occurred when creating CSI namespace")
			return err
		}
	} else {
		err = r.deleteCSINamespace(ctx)
		if err != nil {
			ctx.Logger.Error(err, "Error occurred while deleting CSI namespace")
			return err
		}
		CsiNamespace = "kube-system"
...
func (r *VDOConfigReconciler) createCSINamespace(ctx vdocontext.VDOContext) error {
	nsSpec := &v1.Namespace{
